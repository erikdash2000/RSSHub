        ...
        item:
            ...
            list
                .map((index, item) => {
                    item = $(item);
                    return {
                        title: `${item.find('.game-collection-item-details-title').text()} [${item.find('.game-collection-item-discount').text()}] [${item.find('.game-collection-item-discount-bonus').text()}]`,
                        description: `
                            ${item.find('.game-collection-item-top-platform').text()}<br>
                            ${item.find('.game-collection-item-discount').text()}<br>
                            ${item.find('.game-collection-item-discount-bonus').text()}<br>
                            ${item.find('.game-collection-item-price-discount').text()}<br>
                            ${item.find('.game-collection-item-price-bonus').text()}<br>
                            ${item.find('.game-collection-item-price ').text()}<br>
                            <del>${item.find('span.game-collection-item-price.strikethrough').text()}</del><br>
